---
title: "USGSHydroOpt : Tools for Optical Analysis of Water"
author: "Samuel Christel and Steve Corsi"
output: 
...
```{r array, echo=TRUE, eval=TRUE,tidy=TRUE}
...
```{r  produce array workflow, echo=TRUE, eval=FALSE,tidy=TRUE}
...
```{r getAbs,echo=TRUE, eval=TRUE,tidy=TRUE}
...
```{r getSag, echo=TRUE,eval=FALSE, tidy=TRUE}
...
```{r getExpResid, echo=TRUE,eval=FALSE, tidy=TRUE}
...
```{r getIndexes, echo=TRUE,eval=TRUE, tidy=TRUE}
...
```{r getRatios, echo=TRUE, eval=TRUE, tidy=TRUE}
#assign a variable dataSummary to the dfsummary dataframe included in USGSHydroOpt
dataSummary <- dfsummary
...
```{r getMeanFl, echo=TRUE, eval=TRUE, tidy=TRUE}
...
```{r getLog10, echo=TRUE, eval=TRUE, tidy=TRUE}
...
```{r plotEEMs2,eval=TRUE,echo=TRUE,tidy=TRUE} 
