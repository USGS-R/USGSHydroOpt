...
#' @param baseSig name of the column to subtract out the base signal
    ...
    for(i in 1:(length(sigs)-1)){
      ...
      for(j in (i+1):length(sigs)){
        ...
        if(mean(dataSummary[,varName1],na.rm=TRUE) > (mean(dataSummary[,varName2],na.rm=TRUE))){
          ratio <- (dataSummary[,varName1] - dataSummary[,baseSig])/(dataSummary[,varName2] - dataSummary[,baseSig])
          ratioName <- paste0("rB",varName1,"_",varName2,"_B",baseSig)
          var1 <- c(var1,varName1)
          var2 <- c(var2,varName2)
          
        }else{
          ratio <- (dataSummary[,varName2] - dataSummary[,baseSig])/(dataSummary[,varName1] - dataSummary[,baseSig])
          ratioName <- paste0("rB",varName2,"_",varName1,"_B",baseSig)
